...
public class P001819 {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		PrintStream fl = new PrintStream(new File("POO_1819OF.txt"));
	...
	private static void alinea2(PrintStream out) {
		...
		try(Scanner uafest = new Scanner(new File("uafest.txt"))){
			//Cabeçalho
			String[] data = uafest.nextLine().split(";");	//data=[nome, endereçoMercado]
			fest = new Mercado(data[0], data[1]);
			
			//Restante documento
			Tenda t=null;
			while(uafest.hasNext()) {
				data=uafest.nextLine().split(";");	//CASO BAR data=[codTenda, numero, nome]
				//CASO TASQUINHA data=[codTenda, numero, nome, n*pratos]
				switch(data[0]) {
				case("1"):	//Bar
					t = new Bar(Integer.parseInt(data[1]), data[2]);
					break;
				case("2"):	//Tasquinha
					t = new Tasquinha(Integer.parseInt(data[1]), data[2], Arrays.copyOfRange(data, 3, data.length));
					//Arrays.copyOfRange(String[], indexInclusive, indexExclusive) https://docs.oracle.com/javase/8/docs/api/java/util/Arrays.html#copyOfRange-T:A-int-int-
				}
				fest.add(t);
			}
